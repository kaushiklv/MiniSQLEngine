...
def cartesian_product(from_tables, actual_data):
    ...
    if not final_temp_table:
        final_temp_table = temp_table
        ...
        if isinstance(condition_tuples[0][0], list) and isinstance(condition_tuples[0][1], list):
            ...
            for row in query_results:
                ...
                if valid:
                    temp = []
                    for i, col in enumerate(row):
                        if col_map[i] in select_columns:
                            temp.append(col)
                    final_results.append(temp)
            query_results = final_results
        else:
        ...
        for col in select_columns:
            for table in from_tables:
                if col in list(actual_data[table].keys()):
                    result_columns.append(table + "." + col)
                    break
        return query_results, result_columns
    else:
        aggregate_values = []
        ...
        if isinstance(condition_tuples[0][0], list) and isinstance(condition_tuples[0][1], list):
            ...
            if isinstance(condition_tuples[0][0], list) and isinstance(condition_tuples[0][1], list):
                ...
                for row in query_results:
                    ...
                    valid2 = 0
                    for i, col in enumerate(row):
                        if col_map[i] == condition_tuples[1][0] and perform_operation(col, condition_tuples[1][1], condition_tuples[1][2]):
                            valid2 = 1
                    if valid1 and valid2:
                        temp = []
                        count = 0
                        for i, col in enumerate(row):
                            if col_map[i] in select_columns and count != len(select_columns):
                                temp.append(col)
                                count += 1
                        final_results.append(temp)
                query_results = handle_distinct(final_results, distinct)
